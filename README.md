...
基于 Python，当前集成 Telegram + Codex / Claude / Gemini 三引擎（Codex 为主，Claude / Gemini 可切换）。Long Polling 模式，无需公网 IP 或反向代理，启动即用；当前版本还支持更顺滑的 `/resume` 会话恢复、可展开的 thinking 过程展示、轮询自愈，以及可选的本地 runtime metrics 端点。
...
- Gemini CLI 现已支持 `/model`、`/resume`、图片消息分析，以及 `/status` 中基于本地 session 的“当日各模型对话次数”统计。
...
Bot 使用 Long Polling 模式主动拉取消息，不需要公网 IP 或反向代理。默认优先 Codex 引擎；Claude 与 Gemini 可按需切换作为备选。
...
- Gemini CLI (可选，仅在切换 Gemini 引擎时需要)
...
| `/status [full]` | `/context` 的兼容别名；Gemini 会额外显示本地 session 聚合的“今日各模型对话次数” | 全部（Codex/Gemini 主展示） |
...
- 直接发送文本消息 = 向当前引擎（Claude/Codex/Gemini）下达指令
- 发送文件 = 由当前引擎分析文件内容（支持代码、配置、文档）
- 发送图片 = 引擎分析截图/图表
- Claude：SDK 多模态输入
- Codex：CLI `--image`
- Gemini：CLI 本地暂存图片后，通过 `@image_path` 引用提交
